...
6. **SDK 影子层判定**：底层 SDK/宿主已直接给出某事实（同步返回 / 可订阅事件 / getter / 既有常量）时，自造一层轮询 / 重试 / 守卫 / 状态跟踪去拿同一事实即过度设计——**必须给出 SDK 等价面的行号级证据**，打不开证据的降级为「疑似」不进候选；SDK 确无等价面（真能力缺口）时绕法合理；防御类机制还须验证其防御的外部行为真实会发生，未验证 = 按想象失败处理。事故驱动的防线：只有事故根源已修复的才算合法第二道防线，根源未修而靠补偿机制撑着的（删掉后问题还在）进候选。源头案例见 SSOT §2.4/§2.5。
