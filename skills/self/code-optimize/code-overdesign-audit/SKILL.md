...
description: >-
  ...
  三层证据（可计算信号+语义五问+业务保证映射）→ 杠杆排序 → 候选卡片
...
把「过度复杂设计」变成可裁决的简化候选：每张候选卡片论证三段——牺牲什么保证（小取舍）、省下什么机制（大简化）、为什么核心价值无损（核心价值不变的判据）。设计文档是本 skill 的 SSOT：`<skill_dir>/docs/plans/2026-09-10-code-overdesign-audit-skill.md`（本 skill 内，随仓库分发）。

被触发后先判模式，再按路由 read 对应文档；细节全部在 references，正文不含。

## 核心骨架与演进框架

本节是 skill 的结构地图与自身更新守卫（回答「该不该接受某个更新提案」时以本节为准），不是审计操作细节：

```text
本质：把「感觉这段代码过度了」的直觉，
      变成有证据、可裁决、防幻觉的简化候选清单

┌─ 心脏：判定标准
│    证据判据：语义五问（四问 + SDK 影子层）+ 反模式清单（evidence-signals.md）
│    判定边界：豁免清单（什么不算过度）
│    —— 边界错 = 把合法防线误判成候选，与判据同等重要
│
│    判定标准靠流程被系统执行到全仓库
▼
┌─ 骨架：流程
...
┌─ 可信度
│    语义「投机」结论调用方抽验 ≥1
│    「已核实非过度」附证据快照、计数变化即重验

演进主干 [HISTORICAL]（2026-09-25 提案返工教训）：
  演进单位是「判据」，不是「机制」——
  真实案例暴露判定漏洞 → 修判据或修边界 → 案例登记为出处

扩展点只有两个，准入条件不同：
  ① 判据/边界修正——真实案例暴露判定漏洞即可修
     （修错不需要反复证据；先例：SDK 影子层，SSOT §2.4）
  ② 可计算信号下沉——同类判定反复发生且 grep 模式稳定可靠，
     才把该判定从语义层下沉为可计算信号
     （新增能力需要反复证据；先例：knip 无调用方导出）

禁长方向 [HISTORICAL]（2026-09-25：向本 skill 提 5 处更新、其中 3 处因属此方向被否决）：
  × 来源分类学/理论框架——解释性知识，不改变检测行为
  × 反模式百科化——清单每条必须带判定动作，为全而全不加条目
  × 平行检测管道——判据改了语义层自动跟上，勿重复实现同一判定

更新提案过滤器：这是判据/边界的修正，还是机制堆叠？
  答不出「暴露了哪个判定漏洞」的更新提案默认不收。
```

## 核心原则

1. **先证据后判定**：语义层的每个「投机」判定必须附调用方清单与证据行号，无证据的怀疑丢弃——复杂度高 ≠ 过度设计（可能是本质复杂度），分层转发型过度设计单点复杂度都很低。
6. **SDK 影子层判定**：底层 SDK/宿主已直接给出某事实（同步返回 / 可订阅事件 / getter / 既有常量）时，自造一层轮询 / 重试 / 守卫 / 状态跟踪去拿同一事实即过度设计——**必须给出 SDK 等价面的行号级证据**，打不开证据的降级为「疑似」不进候选；SDK 确无等价面（真能力缺口）时绕法合理；防御类机制还须验证两件事——其防御的外部行为真实会发生，且其检测条件在生产形态输入下命中过（判定键形态与生产实际传入一致、锚定测试用生产可达形态构造；恒不命中 + 测试捏造形态 = 常绿的死防线），任一未验证 = 按想象失败处理。事故驱动的防线：只有事故根源已修复的才算合法第二道防线，根源未修而靠补偿机制撑着的（删掉后问题还在）进候选。源头案例见 SSOT §2.4/§2.5。
...
4. **粗网细网分工**：pre-commit 断言是粗网（宁漏勿拦——宁可漏过错误也不误拦正常项；四类已知漏拦由 audit 细网兜底捕捉）；audit 是细网（knip 精确分析 + 语义五问，不依赖 grep 计数）。
...
2. **三层证据**：read `references/evidence-signals.md`——可计算层（knip/转发占比/单实现接口/链深，bash 并行）+ 语义层（subagent 按五问×模块分组并行扫，派发模板同文件）+ 业务层（读设计文档/ADR 映射保证→调用方）。
...
- [MANDATORY] 派发遵守全局 AGENTS.md：并发 ≤5、单 subagent ≤5 文件/3000 行、task 三段式、模型按全局路由表（扫描用 flash 级、thinking max）、一律后台异步派发靠通知推进。subagent 不自动加载 skill——五问清单与反模式清单必须内联进派发提示词（模板见 evidence-signals.md）。
...
- [OPTIONAL] 报告可粘贴进 issue/ADR 复用；用户裁决后可建议把「已核实非过度」的重要判定登记为 ADR（防未来重复建议）。
