...
- 同构数据族两套传输契约：结构相同的两族实体，一族推送携带完整数据、另一族只发变更信号再让对端回拉全文——昂贵通道恰好没给最需要数据的一族；信号再被登记成状态主题导致多实例互相覆盖的形态同族
- 跨进程/跨层引用以显示名或可变字符串作匹配键：一侧存完整路径/规范 id，另一侧用短名/别名匹配——恒不命中或随机失配（身份键与显示名混用）
