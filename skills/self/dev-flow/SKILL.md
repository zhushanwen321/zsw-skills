...
| 1 执行计划 | `<项目根>/.tmp/dev-flow/<同名>.impl-plan.md`（DAG+单元表+状态表+验收计划表） | 计划自检通过后基线落盘（Write 计划文件即完成，**不 commit、禁 `git add`/`git add -f`**——`.tmp/` 是 gitignored 产物目录，全局 AGENTS.md「提交策略」同款禁令）；**台账全程盘面作业**：状态表/变更历史/验收回填等一切进度登记写盘即闭环，任何时点不产生 commit，需要用户可见的结论落 docs/ 下设计文档随所属功能 commit 入库；验收计划表含每项方式/成本/收益打分与提速结论 |
