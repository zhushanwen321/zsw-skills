...
- [ ] **禁止工具调用的措辞强度测试**：删除所有反工具调用围栏后，模型会不会尝试调工具？如会 → 围栏还不够强。
...
1. **维度覆盖**：5 个设计维度（反工具调用围栏/章节模板/Analysis草稿区/Partial Compact/自定义指令）是否均有检查项？
2. **P0 判断准确**：反工具调用围栏和 Current Work 原文引用的 P0 定位是否合理（缺少确实=压缩失败/任务漂移）？
