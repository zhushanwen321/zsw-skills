...
name: drawio-skill
version: 1.15.0
description: Use when the user requests diagrams, flowcharts, architecture diagrams, ER diagrams, UML / sequence / class diagrams, network topology, ML/DL model figures (Transformer/CNN/LSTM), mind maps, or any visualization. Also use proactively when explaining systems with 3+ components, complex data flows, or relationships that benefit from visual representation. Best suited when the diagram needs custom styling, rich shape vocabulary, swimlanes, or exportable images (PNG/SVG/PDF/JPG). Also use when you need to embed a draw.io diagram into an HTML page — generates a self-contained interactive preview (zoom/layers/lightbox) via `drawio -f html`. Generates .drawio XML and exports locally via the native draw.io desktop CLI.
license: MIT
...
**Supported formats:** PNG, SVG, PDF, JPG, plus a self-contained embeddable HTML viewer — no browser automation needed.
...
# HTML embed — self-contained interactive preview (see "Embed in HTML" below)
drawio -x -f html -o diagram.html input.drawio
...
- `-f` — format: `png`, `svg`, `pdf`, `jpg`, `html` (`html` = self-contained interactive preview page, see "Embed in HTML")
...
### Embed in HTML (interactive preview)

When you need to embed a diagram into a webpage or generate a standalone preview page, use the CLI's `-f html`. It produces a **self-contained** HTML file: the diagram XML is inlined into a `data-mxgraph` attribute and rendered by the official `viewer-static.min.js` (supports zoom, layers, multi-page, lightbox, retina vectors). The CLI handles the JSON/HTML/XML triple-encoding correctly — **do not hand-build the `data-mxgraph` attribute**, HTML attribute decoding silently corrupts XML entities (`&quot;`/`&amp;`) and breaks both JSON and XML.

```bash
# Self-contained interactive preview HTML (XML inlined, viewer JS from CDN)
drawio -x -f html -o diagram.html input.drawio
```

The output loads `viewer-static.min.js` from `https://viewer.diagrams.net/` by default, so the first view needs internet. For **offline / air-gapped** use, download `viewer-static.min.js` once and swap the `<script src>` in the generated HTML to a local path.

**Static preview (no JavaScript):** for a non-interactive embed, export SVG with `-e` and inline the SVG markup directly into the target page — zero JS dependency, works everywhere:

```bash
drawio -x -f svg -e -o diagram.svg input.drawio   # then paste the SVG into the page
```

| Need | Use |
|------|-----|
| Interactive preview (zoom / layers / lightbox) | `drawio -x -f html` |
| Static, no-JS embed | `drawio -x -f svg -e` + inline the SVG |
| Diagram in Markdown / git | export SVG/PNG, reference as an image |
| Public-URL iframe (file hosted online) | draw.io app → File > Embed > IFrame |
