...
[MANDATORY] **此文件是执行指令，不是参考文档。** 你作为 AI agent 必须按步骤 1→6 的顺序消费 pattern 的每个章节，不跳过任何步骤。如果你跳过 create.md 直接 read patterns/，你失去了风险分级、逐维度设计协议和设计走查——三个关键保障。
...
create.md 引导你按固定顺序消费 `patterns/<carrier>.md` 的各章节。每个章节对应设计的一个关键环节，跳过任何一个都意味着设计缺少系统性保障。
